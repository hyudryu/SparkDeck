...
Community inference estimates include only measurements captured while exactly one inference stream is active. C2 and higher runs remain in local benchmark history but are not contributed. SparkDeck removes output-speed outliers and uploads one current average for the signed-in user and exact model, quantization, and prompt-length bucket. The hosted service then averages those per-user values with equal weight, so one busy user cannot dominate the result. A fully local fallback has no account identity and uses the installation's telemetry cluster ID as the contributor proxy. Local calculations are bounded to the latest 100,000 eligible rows and the latest 256 observations per contributor and evidence dimension.
