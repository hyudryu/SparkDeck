...
Community inference estimates include only measurements captured while exactly one inference stream is active. C2 and higher runs remain in local benchmark history but are not contributed. For each user, SparkDeck removes output-speed outliers and averages the remaining measurements for the exact model, quantization, and prompt-length bucket. The community estimate then averages those per-user values with equal weight, so one busy user cannot dominate the result.
