...
**Telemetry upload runs on every paired node.** The community API (`https://oqft567ar3.execute-api.us-east-2.amazonaws.com`, seeded as the default `community_api_url` — clear the Settings field to stay fully local, or override the seed with `SPARKDECK_COMMUNITY_API_URL`) accepts consented samples at `POST /v1/samples` and serves aggregates at `GET /v1/aggregates`, both authenticated with a Cognito ID token. To let uploads run without a browser open, sign-in also stores the Cognito **refresh token** in the node's local pairing record (it is never echoed in API responses or `sync_status`, and it is used solely to mint short-lived ID tokens for telemetry upload; the frontend's sign-out revokes it at Cognito). A background uploader ticks every 60 seconds while sharing is opted in: it refreshes an ID token as needed, posts each pending outbox sample — exactly `model_id`, `context_window_size`, and `inference_tokens_per_second` — and marks results `synced`, leaves transient failures pending, and marks definitive rejections `failed` (retryable from the Benchmarks page). Community estimates are proxied through `GET /api/v1/community/aggregates`, which forwards the browser's token to the live API.
