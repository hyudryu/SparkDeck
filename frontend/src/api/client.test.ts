...
describe('API client adapters', () => {
  ...
  it('lets update readiness outlive the backend preflight budget', async () => {
    vi.useFakeTimers()
    const fetchMock = vi.fn<typeof fetch>().mockImplementation((_input, init) => new Promise((_resolve, reject) => {
      init?.signal?.addEventListener('abort', () => reject(new DOMException('Aborted', 'AbortError')), { once: true })
    }))
  ...
  it('preserves aggregate evidence dimensions without adding device or runtime fields', async () => {
    const response = {
      items: [{ model_id: 'org/model', quantization: 'NVFP4', tensor_parallel_size: 4, prompt_tokens_bucket: 1000, inference_tokens_per_second: 28.5, sample_count: 11, unique_cluster_count: 4 }],
  ...
  it('saves node preferences for llama.cpp deployments like the other runtimes', async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify({
      ...
      model: { repository: 'org/model', artifact: 'Q4_K_M/model.gguf' }, status: 'saved', settings: {},
    }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
    vi.stubGlobal('fetch', fetchMock)

    await api.deployments.create({
      alias: 'local-gguf', model_id: 'org/model', runtime: 'llama.cpp', managed: true,
      settings: { context_length: 8192, gpu_layers: 99, artifact: 'Q4_K_M/model.gguf' },
      node_ids: ['local', 'spark-2'], deployment_mode: 'replicated',
    })
    ...
    expect(body).not.toHaveProperty('download_node_id')
  })
  ...
  it('loads persisted usage and requests an authoritative cluster sync', async () => {
    const fetchMock = vi.fn<typeof fetch>()
