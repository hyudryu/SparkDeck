...
export function BenchyChart({ title, subtitle, rows, metric }: BenchyChartProps) {
  ...
  const concurrencyLevels = [...new Set(measured.map((row) => row.concurrency ?? 1))].sort((a, b) => a - b)
    ...
    <section className="benchmark-chart" aria-label={title}>
      <div className="benchmark-chart-heading">
        ...
        <div className="benchmark-chart-legend" aria-label="Concurrency legend">
          {concurrencyLevels.map((level, index) => (
            <span key={level}><i className={`chart-color-${index % 6}`} />C{level}</span>
          ))}
        </div>
      </div>
      {measured.length === 0 || promptSizes.length === 0 ? <div className="benchmark-chart-empty">No measured points for this run yet.</div> : <>
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-labelledby={`${metric}-chart-title ${metric}-chart-desc`}>
          ...
          <desc id={`${metric}-chart-desc`}>{subtitle} One line per concurrency level across prompt sizes.</desc>
          ...
          {concurrencyLevels.map((level, colorIndex) => {
            const values = new Map(
              measured
                .filter((row) => (row.concurrency ?? 1) === level)
                .map((row) => [row.prompt_size ?? 0, row]),
            )
            const points = promptSizes
              .filter((size) => values.has(size))
              .map((size) => ({
                size,
                x: x(size),
                y: y(values.get(size)![metric] as number),
                value: values.get(size)![metric] as number,
              }))
            const segments: typeof points[] = []
            let segment: typeof points = []
            promptSizes.forEach((size) => {
              const point = points.find((item) => item.size === size)
              if (point) segment.push(point)
              else if (segment.length) { segments.push(segment); segment = [] }
            })
            if (segment.length) segments.push(segment)
            return <g className={`chart-series chart-color-${colorIndex % 6}`} key={level}>
              ...
              {points.map((point) => <circle key={point.size} cx={point.x} cy={point.y} r="4">
                <title>
                  {compactTokens(point.size)} prompt, C{level}: {point.value.toFixed(1)} tok/s
                </title>
              </circle>)}
            </g>
          })}
        ...
        <table className="sr-only">
          ...
          <thead><tr><th>Prompt size</th><th>Concurrency</th><th>Tokens per second</th></tr></thead>
          <tbody>
            {measured.map((row, index) => <tr key={index}>
              <td>{row.prompt_size}</td><td>{row.concurrency}</td><td>{row[metric]}</td>
            </tr>)}
          </tbody>
        </table>
      </>}
