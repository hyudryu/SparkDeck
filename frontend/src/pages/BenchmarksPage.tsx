...
export function BenchmarksPage() {
  ...
  return (
    <div className="page">
      ...
      <div className="section-heading" title={communityAccess.enabled ? undefined : accessHint}><div><h2>{localAggregates ? 'Local aggregate estimates' : 'Community estimates'}</h2><p>Evidence is matched only by exact model name, quantization, and context window. Results are estimates, not guarantees.</p></div></div>
      ...
      />}
      {communityAccess.enabled && !aggregates.loading && !aggregates.error && aggregateResponse?.availability !== 'unavailable' && aggregateResponse?.items.length === 0 && <EmptyState title="No community estimates yet" description="Estimates will appear when enough samples share the same model name, quantization, and context window." />}
      {communityAccess.enabled && aggregateResponse && aggregateResponse.items.length > 0 && <div className="aggregate-grid">{aggregateResponse.items.map((item) => (
        <Panel className="aggregate-item" key={`${item.model_id}-${item.quantization}-${item.context_window_size}`}>
      ...
      {communityAccess.enabled && aggregateResponse && <p className="aggregate-policy">Evidence threshold: {aggregateResponse.evidence_policy.minimum_samples} samples, matched only on model name, quantization, and context window. Inference speed is {localAggregates ? 'aggregated from this controller' : 'a community estimate'} and may differ on your system.</p>}
