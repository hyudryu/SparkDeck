      ...
      {status && <>
        ...
        {controller && joining && <Panel className="join-panel">
          ...
          <form onSubmit={(event) => void join(event)}>
            <div className="field"><label htmlFor="controller-tailnet-url">Existing cluster entry URL</label><input id="controller-tailnet-url" type="url" required aria-describedby="controller-tailnet-url-help" value={form.controller_url} onChange={(event) => setForm({ ...form, controller_url: event.target.value })} placeholder="https://spark-entry.your-tailnet.ts.net" /><small id="controller-tailnet-url-help">Use a Tailscale HTTPS URL shown by the controller or an online worker.</small></div>
