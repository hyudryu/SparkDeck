...
export function DeploymentPage() {
  ...
  const requiredRunNodes = () => {
    ...
    if (mode === 'replicated' || mode === 'grouped_sharded') {
      // Both persisted layouts publish an exact node-count contract:
      // replicas need their full copy count, grouped-sharded needs the
      // whole instances × TP topology (per-group TP is not a world size,
      // so the sharded argv math below does not apply).
  ...
  // Grouped-sharded deployments toggle one engine group without touching the
  // others; the deployment's own state is derived on the backend.
  const actOnInstance = async (instance: number, action: 'start' | 'stop') => {
    ...
    try {
      ...
      setNotice(`Instance ${instance} ${action === 'start' ? 'starting' : 'stopping'}…`)
      // Reflect the acted group immediately (the response carries the derived
      // status), then re-sync with the authoritative detail.
      resource.setData((current) => current ? {
        ...current,
        status: updated?.status ?? current.status,
        instances: current.instances?.map((entry) => entry.instance_id === instance
          ? {
            ...entry,
            desired_state: action === 'start' ? 'running' as const : 'stopped' as const,
      ...
      setError(reason instanceof Error ? reason.message : `Could not ${action} instance ${instance}`)
      resource.reload()
    } finally {
      setBusy(undefined)
    }
  ...
  return <div className="page">
    ...
    <Panel className="settings-section deployment-detail-section">
      ...
      <form className="settings-fields" onSubmit={(event) => void save(event)}>
        ...
        {detail.instances && detail.instances.length > 0 && <div className="wide-field">
          <span className="muted">Engine groups</span>
          ...
          <small>Each group is an independent sharded engine; requests load-balance across the running groups. Stop a group to take it out of rotation without touching the rest.</small>
        </div>}
