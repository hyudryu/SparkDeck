...
class SparkDeckService:
    ...
    async def deployment_detail(self, deployment_id: str) -> dict[str, Any]:
        ...
        # Hook-backed containers carrying an env-file label are edited by
        # writing that file, never by recreating the container, so the usual
        # argv-parse "editable" gate does not apply to them.
        edit_mode = None
    ...
    async def _update_discovered_deployment(
        ...
        if (
            ...
            # Hook-backed containers are regenerated by external scripts from
            # their env file; recreating the container here would either fail
            # argv validation or be discarded by the next hook run.
