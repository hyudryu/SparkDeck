...
from .live_metrics import LiveHistory
...
class SparkDeckService:
    def __init__(self, manager: Any, data_dir: Path):
        ...
        # and its sampler starts lazily when the panel first asks for data.
        self.history = LiveHistory(self.manager)
    ...
    async def deployment_detail(self, deployment_id: str) -> dict[str, Any]:
        ...
        # Hook-backed containers use the same launch-settings editor as every
        # other discovered runtime. Saving takes over the stopped container
        # and removes its external lifecycle labels so a later hook cannot
        # regenerate it with stale, hard-coded settings.
        edit_mode = None
        settings_env: dict[str, Any] | None = None
        if (
            not discovered_editable
            and container
            and str(container.get("settings_env_file") or "").strip()
            and (
                str(container.get("start_command") or "").strip()
                or str(container.get("stop_command") or "").strip()
            )
        ):
            # Keep the legacy env-file fallback only for commands that cannot
            # be parsed into SparkDeck's normal launch-settings contract.
            edit_mode = "env-file"
            editable = True
            edit_reason = None
            settings_env = self._settings_env_payload(
                str(container["settings_env_file"]).strip(),
            )
    ...
    async def _update_discovered_deployment(
        ...
        if external_lifecycle and not settings.get("editable"):
            return await self._update_envfile_backed_deployment(container, changes)
        ...
        await self.manager.update_container_settings(
            name, replacement,
            detach_external_lifecycle=external_lifecycle,
        )
        return await self.deployment_detail(f"container:{name}")

    async def _update_envfile_backed_deployment(
        self, container: dict[str, Any], changes: dict[str, Any],
    ) -> dict[str, Any]:
        """Write a hook-backed card's settings into its labeled env file.

        The container itself is never recreated here; the external start
        script regenerates it from the env file on the next stop/start.
        """
        name = str(container.get("name") or "")
        env_path = str(container.get("settings_env_file") or "").strip()
        if not env_path:
            raise ValueError(
                "this deployment's lifecycle is managed by external scripts; "
                "add an io.sparkdeck.env-file label pointing at its settings "
                "env file to enable editing"
            )

        allowed = {
            "launch_controls", "environment", "env_file_mtime",
            "gpu_memory_utilization", "served_model_name",
            # Rejected below with hook-specific guidance.
            "extra_args", "gpu_memory_gb", "sg_tp_size", "sg_mem_fraction",
        }
        unknown = sorted(set(changes) - allowed)
        if unknown:
            raise ValueError(f"unsupported field(s): {', '.join(unknown)}")
        if changes.get("extra_args"):
            raise ValueError(
                "launch flags are generated by the external start script; "
                "edit the backing variable in the environment section instead"
            )
        if changes.get("gpu_memory_gb") not in (None, ""):
            raise ValueError(
                "GPU memory reserve is not supported for hook-backed deployments"
            )
        if changes.get("sg_tp_size") not in (None, ""):
            raise ValueError(
                "SGLang TP size is not supported for hook-backed deployments"
            )
        if changes.get("sg_mem_fraction") not in (None, ""):
            raise ValueError(
                "SGLang mem fraction is not supported for hook-backed deployments"
            )
        ...
        if expected_mtime is not None and not isinstance(
            ...
            raise ValueError("env_file_mtime must be a number")
        try:
            apply_env_updates(env_path, updates, expected_mtime=expected_mtime)
        except EnvFileConflictError as exc:
            raise ValueError(
                "settings file changed on disk; reload and retry"
            ) from exc

        detail = await self.deployment_detail(f"container:{name}")
