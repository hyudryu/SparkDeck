...
class ServedModelNameTests(unittest.IsolatedAsyncioTestCase):
    ...
    def shell_wrapped_container_summary(self) -> dict:
        manager = Manager.__new__(Manager)
        container = SimpleNamespace(
            short_id="d933706ebaa3",
            name="vllm-qwen35",
            ...
            attrs={
                "Config": {
                    ...
                    "Cmd": [
                        "bash", "-lc",
                        f"exec vllm serve {BACKING_MODEL} "
                        f"--served-model-name ${{SERVED_MODEL_NAME:-{SERVED_MODEL}}} "
                        "--max-model-len 262144",
                    ],
                },
                "Created": "2026-08-03T00:00:00Z",
    ...
    def test_shell_wrapped_container_recovers_served_model_id(self) -> None:
        summary = self.shell_wrapped_container_summary()
    ...
    async def test_models_endpoint_advertises_shell_wrapped_served_name(self) -> None:
        manager = Manager.__new__(Manager)
        manager.get_state = mock.AsyncMock(return_value={
            "containers": [self.shell_wrapped_container_summary()],
            "unsloth": {},
            "sparkrun_targets": {},
        })
    ...
    async def test_shell_wrapped_backing_model_request_is_rewritten_for_vllm(self) -> None:
        manager = Manager.__new__(Manager)
        container = self.shell_wrapped_container_summary()
        manager._resolve_vllm_target = mock.AsyncMock(return_value=container)
        response = mock.Mock()
        response.raise_for_status.return_value = None
        response.json.return_value = {"choices": [], "usage": {}}
