...
class DiscoveredDeploymentDetailTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rename_discovered_container_updates_display_alias(self):
        ...
        with (
            ...
            patch.object(server.sparkdeck.store, "deployment", Mock(return_value=None)),
            patch.object(server.manager, "update_container_alias", rename),
            patch.object(server.manager, "container_aliases", {}),
        ):
            response = await self.client.patch(
    ...
    async def test_rename_discovered_container_rejects_alias_conflicts(self):
        card = {
            "id": "container:vllm-dspark", "alias": "dspark", "runtime": "vllm",
            "kind": "external", "model": {"repository": "org/model"},
            "status": "stopped", "settings": {},
        ...
        with (
            ...
            patch.object(server.manager, "update_container_alias", rename),
            patch.object(
                server.manager, "container_aliases", {"other-container": "Vision exp"},
            ),
        ):
            response = await self.client.patch(
                "/api/v1/deployments/container:vllm-dspark",
    ...
    async def test_rename_discovered_container_rejects_saved_deployment_alias(self):
        ...
        with (
            ...
            patch.object(
                server.sparkdeck.store, "deployment",
                Mock(return_value={"id": "saved-1", "alias": "Saved model"}),
            ),
            patch.object(server.manager, "update_container_alias", rename),
            patch.object(server.manager, "container_aliases", {}),
        ):
