...
@pytest.mark.asyncio
async def test_update_waits_for_both_export_and_import_and_rejects_new_streams(tmp_path):
...
@pytest.mark.asyncio
async def test_existing_job_can_open_its_stream_after_update_is_reserved(tmp_path):
    service = nas(tmp_path)
    begin = asyncio.Event()
...
@pytest.mark.asyncio
async def test_admitted_peer_operation_drains_and_releases_on_failure(tmp_path):
    service = nas(tmp_path)
    opened = asyncio.Event()
    finish = asyncio.Event()
...
@pytest.mark.asyncio
async def test_queued_transfer_stays_queued_until_update_releases(tmp_path):
    service = nas(tmp_path)
